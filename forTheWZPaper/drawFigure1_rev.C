...
// drawFigure1
//------------------------------------------------------------------------------
void drawFigure1_rev(TString energy = "8TeV")
{
  ...
  TFile * file;
  if (energy.Contains("7TeV")) {
    file = new TFile("rootfiles/" + name + energy + ".root", "read");
  } else {
    file = new TFile("/users/ltikvica/CMSSW_4_2_9_HLT1/src/latinosAnalysis/rezultati/rootFiles/forPlot.root", "read");
  }
  TH1F* data;
  TH1F* WZ;
  TH1F* fakes;
  ...
  if (energy.Contains("7TeV"))
    {
      ...
      allmcbg = (TH1F*) ZZ->Clone("hZmass_Sel_mcbg");
      allmcbg->Add(Zgamma);
      VVV = (TH1F*) allmcbg->Clone("hZmass_Sel_vvv");

      float nmcbg_ini   = allmcbg->Integral();
      float nvvv        = 9.0;
      VVV->Scale(9.0/nmcbg_ini);
      std::cout << "7 TeV MC  normalization before : " << allmc->Integral() << std::endl;
      allmcbg->Add(VVV);
      if (newPlot) allmc->Add(VVV);
      std::cout << "7 TeV MC bg  normalization after : " << allmc->Integral() << std::endl;
    }
  else if (energy.Contains("8TeV"))
    {
      ...
      allmcbg = (TH1F*) ZZ->Clone("hZmass_allmcbg");
      allmcbg->Add(Zgamma);
      allmcbg->Add(WV);
      allmcbg->Add(VVV);
    }
  ...
  if (newPlot) {
    ...
    allmcbg->SetFillColor(kRed+1);
    allmcbg->SetLineColor(kRed+1);
  ...
  if (newPlot) {
    DrawTLegend(x0, y0 - 1.*(_yoffset+0.001), allmcbg," MC bkg",            "f");
  } else {
  ...
  if (energy.Contains("7TeV"))
    {
      float err_offset = 3.;
      if (newPlot) err_offset = 2.;
      //      DrawTLegend(x0, y0 - 3.*(_yoffset+0.001), allmc, " stat #oplus syst", "f");
      DrawTLegend(x0, y0 - err_offset*(_yoffset+0.001), allmc, " stat #oplus syst", "f");
    }
  else
    {
      if (newPlot) {
