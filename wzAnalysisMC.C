...
int main()
{
  ...
  */
  ...
  const float N_selectedEEE(231);
  const float N_bkgEEE(16);
  const float N_selectedEEM(286);
  const float N_bkgEEM(42);
  const float N_selectedEMM(400);
  const float N_bkgEMM(55);
  const float N_selectedMMM(556);
  const float N_bkgMMM(85);
  float numZ(0), numW(0), numMET(0), num3e(0), num2e1mu(0), num1e2mu(0), num3mu(0), numMET3e(0), numMET2e1mu(0), numMET1e2mu(0), numMET3mu(0);
  ...
  float crossSectionEEE, crossSectionEEM, crossSectionEMM, crossSectionMMM;
  float tauFactor;
  ...
  //inputName.push_back("/STORE/lucija/latinosTrees/MC_LooseLooseTypeI/latino_074_WZJetsMad_GenVars.root");
  //inputName.push_back("/STORE/lucija/latinosTrees/MC_LooseLooseTypeI/latino_074_WZJetsMad.root");

  readFileFromList("wzNoFilter.files", &inputName);
  //  readFileFromList("wzNoFilter.files", &inputName);
  ...
  for  (Int_t k = 0; k<events /*&& k<1000*/;k++) {
    ...
    if (ev3e){
      ...
      myfile3e<<cWZ->run<<":"<<cWZ->event<<":"<<pileUpWeight<<":"<<ScaleFactors(MuonSF, ElecSF, 0, WZcandidates, pt, eta)<<":"<<TriggerWeight(WZcandidates, DoubleElLead, DoubleMuLead, DoubleElTrail, DoubleMuTrail, 0, pt, eta)<<":"<<std::endl;
    ...
    if (ev2e1mu){
      ...
      myfile2e1mu<<cWZ->run<<":"<<cWZ->event<<":"<<pileUpWeight<<":"<<ScaleFactors(MuonSF, ElecSF, 1, WZcandidates, pt, eta)<<":"<<TriggerWeight(WZcandidates, DoubleElLead, DoubleMuLead, DoubleElTrail, DoubleMuTrail, 1, pt, eta)<<":"<<std::endl;
    }
    ...
    if (ev1e2mu){
      ...
      myfile1e2mu<<cWZ->run<<":"<<cWZ->event<<":"<<pileUpWeight<<":"<<ScaleFactors(MuonSF, ElecSF, 2, WZcandidates, pt, eta)<<":"<<TriggerWeight(WZcandidates, DoubleElLead, DoubleMuLead, DoubleElTrail, DoubleMuTrail, 2, pt, eta)<<":"<<std::endl;
    }
    ...
    if (ev3mu){
      ...
      myfile3mu<<cWZ->run<<":"<<cWZ->event<<":"<<pileUpWeight<<":"<<ScaleFactors(MuonSF, ElecSF, 3, WZcandidates, pt, eta)<<":"<<TriggerWeight(WZcandidates, DoubleElLead, DoubleMuLead, DoubleElTrail, DoubleMuTrail, 3, pt, eta)<<":"<<std::endl;
    }
  ...
  std::cout<<"3e:   "<<sqrt(errorAxe3e/(numMET3eGEN*numMET3eGEN))<<std::endl;
  std::cout<<"2e1mu "<<sqrt(errorAxe2e1mu/(numMET2e1muGEN*numMET2e1muGEN))<<std::endl;
  std::cout<<"1e2mu "<<sqrt(errorAxe1e2mu/(numMET1e2muGEN*numMET1e2muGEN))<<std::endl;
  std::cout<<"3mu   "<<sqrt(errorAxe3mu/(numMET3muGEN*numMET3muGEN))<<std::endl;
  float denominator=numMET3eGEN+numMET2e1muGEN+numMET1e2muGEN+numMET3muGEN+numMETSomethingGEN;

  std::cout<<"////AxE in Spanish way"<<std::endl;
  std::cout<<"AxE3e:    "<<numMET3e/denominator <<std::endl;
  std::cout<<"AxE2e1mu: "<<numMET2e1mu/denominator  <<std::endl;
  std::cout<<"AxE1e2mu: "<<numMET1e2mu/denominator  <<std::endl;
  std::cout<<"AxE3mu  : "<<numMET3mu/denominator<<std::endl;

  std::cout<<"errorSpanish"<<std::endl;
  std::cout<<"3e:   "<<sqrt(errorAxe3e/(denominator*denominator))<<std::endl;
  std::cout<<"2e1mu "<<sqrt(errorAxe2e1mu/(denominator*denominator))<<std::endl;
  std::cout<<"1e2mu "<<sqrt(errorAxe1e2mu/(denominator*denominator))<<std::endl;
  std::cout<<"3mu   "<<sqrt(errorAxe3mu/(denominator*denominator))<<std::endl;
  /*  
  std::cout<<"////////GENERATED NUMBERS///////////////"<<std::endl;
  std::cout<<"3e:   "<<numGEN3e<<std::endl;
  std::cout<<"2e1mu:"<<numGEN2e1mu<<std::endl;
  std::cout<<"1e2mu:"<<numGEN1e2mu<<std::endl;
  std::cout<<"3mu:  "<<numGEN3mu<<std::endl;
  std::cout<<"the rest:"<<numGENSomething<<std::endl;
  std::cout<<"sum:     "<<numGEN3e+numGEN2e1mu+numGEN1e2mu+numGEN3mu+numGENSomething<<std::endl;
  */
