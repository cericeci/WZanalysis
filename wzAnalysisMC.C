...
double deltaRWleptMET(int * WZcandidates, TLorentzVector* analysisLepton, TLorentzVector EventMET);
...
int main()
{
  ...
  */
  ...
  TH1F * hBrWeights= new TH1F ("hBrWeights", "hBrWeights", 25, 0.9, 1.15); 
...
double gen[5], genq[5], fact[5], factq[5], mixed[5], error[5], errorWholeZagreb[5], errorRest[5], errorSpanish[5];
  ...
  double numberPileupBrWeights(0.0);
  ...
  for  (Int_t k = 0; k<events /*&& k<2000*/;k++) {
    ...
    numberPileupBrWeights+=pileUpWeight*weightBr;
    
    hBrWeights->Fill(weightBr);
	...
	//	myfile[ch]<<cWZ->run<<":"<<cWZ->event<<":"<<pileUpWeight<<":"<<ScaleFactors(MuonSF, ElecSF, 0, WZcandidates, pt, eta)<<":"<<triggerEff<<":"<<std::endl;      
      }
    }
    //filling aTGC tree
  ...
  ///*********OUTPUT
  //some tests:
  std::cout<<"FOR NORMALIZATION: "<<"pileup: "<<numberPileUp<<" , withoutpileup:"<<numberPileupDown<<" ,withbrWeight:"<<numberPileupBrWeights<<std::endl;
  ...
  */
  ...
  std::cout<<"3e: "<< numMET_brCorr[0]*xs_weight<<"+/-"<<sqrt(numMET_brCorr2[0])*xs_weight<<std::endl;
  std::cout<<"2e1mu: "<<numMET_brCorr[1]*xs_weight<<"+/-"<<sqrt(numMET_brCorr2[1])*xs_weight<<std::endl;
  std::cout<<"1e2mu: "<<numMET_brCorr[2]*xs_weight<<"+/-"<<sqrt(numMET_brCorr2[2])*xs_weight<<std::endl;
  std::cout<<"3mu: "<<numMET_brCorr[3]*xs_weight<<"+/-"<<sqrt(numMET_brCorr2[3])*xs_weight<<std::endl;
